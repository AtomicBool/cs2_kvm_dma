...
ball2_pos = (ball1_pos[0] + random.randint(-400, 400),ball1_pos[1] + random.randint(-400, 400))
...
def mouse_click(event):
    ...
    elif event.x >= ball2_pos[0] - ball_radius and event.x <= ball2_pos[0] + ball_radius and event.y >= ball2_pos[1] - ball_radius and event.y <= ball2_pos[1] + ball_radius and recording:
        recording = False
        canvas.delete("ball2")
...
def save_to_csv(path):
    ...
    # 选择关键点
    key_points_indices = [int(i) for i in range(0, len(path), max(1, len(path)//quantity_points))]
    ...
    with open(csv_file_path, mode='a', newline='') as csv_file:
        ...
        # 写入一行数据
        csv_writer.writerow([f"{key_points_x[-1]},{key_points_y[-1]}"] + [f"{key_points_x[i]},{key_points_y[i]}" for i in range(0,quantity_points - 1)] + [f"{key_points_x[-1]},{key_points_y[-1]}"])
