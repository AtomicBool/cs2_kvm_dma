...
dx_dy_labels_train = train_csv.apply(lambda row: [list(map(int, row[i].split(','))) for i in range(1,21)], axis=1) #修改 为点数+1
dx_dy_test = test_csv.iloc[:, 0].apply(lambda x: list(map(int, x.split(','))))
dx_dy_labels_test = test_csv.apply(lambda row: [list(map(int, row[i].split(','))) for i in range(1,21)], axis=1) #修改
...
class SimpleNet(nn.Module):
    ...
    def forward(self, input_data):
        ...
        x = nn.ReLU()(x)
        x = self.fc3(x)
        return x.view(-1, 20, 2) #输出为 20 * 2 (40)
...
dummy = torch.randn(1, 1, 2) #输入为1*1*2
