...
uint64_t Memory::GetModuleAddress(const char* proc_name, const char* module_name){
	Kernel *tmp_kernel = kernel_clone(kernel);
	auto csgo_process = kernel_into_process(tmp_kernel, proc_name);
	auto info = process_module_info( csgo_process , module_name);
